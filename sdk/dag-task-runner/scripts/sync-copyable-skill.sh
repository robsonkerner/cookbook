...
cat > "$DEST/scripts/package.json" <<'EOF'
{
  ...
  "scripts": {
    ...
    "example": "mkdir -p .canvas/dag-example-workdir && tsx run_dag.ts --dag ../examples/example_dag.json --canvas-path \"$PWD/.canvas/dag-example.canvas.tsx\" --cwd \"$PWD/.canvas/dag-example-workdir\""
  },
  "dependencies": {
    "@cursor/sdk": "^1.0.9"
  },
